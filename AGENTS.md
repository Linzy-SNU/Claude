...
> 이 파일은 [agents.md](https://agents.md) 규약을 따른다. Claude Code · Cursor · Copilot ·
> Gemini CLI 등 **어떤 AI든 이 파일을 읽는다.** `CLAUDE.md`는 이 파일을 가리키는 심볼릭 링크다.
> 규칙을 고칠 때는 **이 파일만** 고친다.

## ⚠️ 세션을 시작하면 먼저 할 것

1. **`docs/context/` 세 파일을 읽는다** — 지금 상태가 여기 있다
   - `docs/context/사용자.md` — 누구인지, 어떻게 대해야 하는지
   - `docs/context/인프라.md` — 무엇을 어디에 두는지
   - `docs/context/작업방식.md` — 어떤 순서로 일하는지
2. 왜 그렇게 됐는지 알아야 하면 `docs/decisions/`를 본다 (날짜순 결정 이력)
3. 진행 중인 일은 `projects/*/handoff.md`

**이 파일에는 규칙만 둔다. 상태와 이력은 `docs/`에 둔다.**
그래야 이 파일이 계속 길어지지 않는다.

> `docs/context/현재작업.md`는 **세션 시작 시 자동으로 주입된다**
> (`scripts/session-context.sh`). 읽으라는 지시에 의존하지 않는다.

## ⚠️ 세션을 끝내거나 중단할 때 할 것

작업이 끝나거나 도중에 멈추면 **반드시** 아래를 남긴다. 이게 없으면 다음 세션이 이어받지 못한다.

1. **`docs/context/현재작업.md` 갱신** — 어디까지 했나 · 다음 할 것 · 막힌 것
   - **40줄 이내로 유지한다.** 끝난 항목은 지우고 상세는 `decisions/`나 `handoff.md`로 옮긴다
   - 매 세션 컨텍스트에 주입되므로 길면 그만큼 손해다
2. **규칙·구조를 바꿨으면** `docs/decisions/YYYY-MM-DD_제목.md`에 근거와 함께 기록
3. **커밋 메시지는 의미 있게 쓴다** — 무엇을 왜 했는지.
   `git log`가 곧 작업 이력이다. 훅이 만드는 `자동 저장: …`는 안전망일 뿐,
   그것에 맡기지 말고 직접 커밋한다
...
| 저장소 | 역할 |
|---|---|
| **GitHub 저장소** | 진실의 원천 — 텍스트·기록·스킬·결정 로그 |
| **iCloud `05.Claude_Output/`** | **저장소** — 완성 문서 원본(.docx/.pdf) |
| **Notion** | **문서 검색·공유용** — 최종본의 요약과 위치만 등록 |
| Google Drive | 학교 제출·공유가 필요할 때만 |
| Dropbox / OneDrive | **손대지 않는다** (기존 개인 파일 보관 중) |

- **문서는 Word + PDF 두 형식으로 동시에** 만든다 → `document-publish` 스킬
- **Notion 등록은 반드시 물어본다.** 최종본인지 확인하고 승인받은 것만 올린다
  (초안까지 올라가면 검색용으로 못 쓴다)
- **대화에서 정한 것은 `docs/`에 기록한다** — 계정·AI가 바뀌어도 `git pull`로
  맥락을 알 수 있게. `docs/context/`(현재 상태) · `docs/decisions/`(결정 이력)
